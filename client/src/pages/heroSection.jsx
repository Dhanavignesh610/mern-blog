...
export default function heroSection({posts}) {
    return(
        ...
        {posts.slice(0,1).map( (post) => (
            ...
            <div className="bg-center bg-no-repeat rounded flex items-end h-[280px] md:h-[380px] bg-gray-700 bg-blend-multiply cursor-pointer bg-cover hover:bg-gray-500  transition ease-in-out" key={`${post._id}`} style={{ backgroundImage: `url(${post.image})`}}>
             <div className="pb-4 px-8">
              <h5 className="text-2xl font-semibold pb-2 leading-none text-white md:text-3xl lg:text-4xl">{post.title}</h5>
        ...
        {posts.slice(1,3).map((post) => (
         ...
         <div className="bg-center bg-no-repeat rounded flex items-end h-[280px] md:h-[184px] w-full bg-gray-700 bg-blend-multiply cursor-pointer bg-cover hover:bg-gray-500  transition ease-in-out" key={`${post._id}`} style={{ backgroundImage: `url(${post.image})`}}>
